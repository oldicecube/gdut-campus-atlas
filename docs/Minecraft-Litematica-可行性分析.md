...
**进展（2026-09-24，本文撰写后已完成）**：`makeCampusModel()` 现在整合同一入口产出地形、道路、水面、程序化树木与全部已建建筑，无需自建体素化管线即可用 GLB → ObjToSchematic 路线落地。全校园 3:1 已按 3×2 网格分片全量跑通（6 片、共 32,236,199 个非空气方块、逐体素零误差），产物与拼接表见 `deliverables/full-campus-3x/`，实测细节见 [`Minecraft-转换实测.md`](./Minecraft-转换实测.md) §3.2。残留的“语义调色板”与 Anvil 世界存档仍是增量工作。
...
**实测校准（3 方块/米，全静态校园，2026-09-24）**：包围盒约 **4431×142×3106 ≈ 19.5 亿** 单元，`.litematic` 若整校单文件按体积分配会直接爆内存；但实际体素化只写表面，整校非空气方块合计仅 **32,236,199**（约 3200 万），分片 `.litematic` 合计约 12.4 MB。**真正的单文件瓶颈不是校园面积，而是 ObjToSchematic 单个进程的 `VoxelMesh` 哈希表**（`Map` + 32 位哈希，约 1600 万条目即 `RangeError`）。据此 3×2 网格分片，每片 ≤3.3 亿网格单元、≤920 万非空气方块，全部跑通。

推荐：

- **全校园总览：推荐 3 方块/米**（3:1）。实测在 3:1 下整个大学城校园合并约 4431×142×3106、仅 3200 万非空气方块，6 个分片即可覆盖；比 1:1（约 14.7 亿单元）低一个数量级，且建筑已足够可辨识。
- 分区/重点轴线：可取 4 单位/方块或更粗，用于远景 LOD 或性能受限的客户端。
- 单体建筑/重点地标：**1 单位/方块**（每米 1 方块）做精修，比例仍与全校园网格一致，可单独投影。
- 不建议把整校做成**单个** `.litematic`/`.schem` 文件；应分片 + 共享原点（`pasteOrigin`）拼接。
...
**结论已从“建议立项”推进到“已全量交付”**：整校 GLB → ObjToSchematic → Sponge `.schem` → `.litematic` 的路线已用 3×2 分片全量跑通。后续增量是可选的原生语义体素化与 Anvil 世界存档，而非阻塞项。
...
**复核（2026-09-24）与最初计划对照**：

1. 已用 `scripts/export-models.ts` / `scripts/export-static-shards.ts` 导出 GLB，经 `ObjToSchematic` 得到 `.schem`，再转 `.litematic`——“几何能进 Minecraft”已从原型升级为全量。
2. 单栋比对（library 等 5 处）与整校分片均已产出，见 `deliverables/` 与 `deliverables/full-campus-3x/`。
3. 已在 `tools/verify-litematic.py`（litemapy 全量逐体素）与 `tools/verify-schem-enginehub.ts`（EngineHub SchematicJS）双重校验，全部 `mismatchCount: 0`。Litematica 实机导入仍建议人工抽验一次。
4. 全校园总览已按 3 方块/米（3:1）分片完成，而非 2 单位/方块；分片 + 共享原点 + `pasteOrigin` 的拼合表在 `manifest.json`。
5. 剩余工作：`.litematic` 直出已实现；**无 mod 的 Anvil 世界存档**尚未做（可经 WorldEdit `//paste` 或自建 Anvil 写出）。
